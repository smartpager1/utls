...
func checkUTLSFingerPrintClientHello(t *testing.T, clientHelloID ClientHelloID, serverName string) {
	uconn := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, clientHelloID, false)
	if err := uconn.BuildHandshakeState(); err != nil {
		t.Errorf("got error: %v; expected to succeed", err)
	}

	generatedUConn := UClient(&net.TCPConn{}, &Config{ServerName: "foobar"}, HelloCustom, false)
...
func TestUTLSFingerprintClientHelloBluntMimicry(t *testing.T) {
	...
	uconn := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, HelloCustom, false)
...
func TestUTLSFingerprintClientHelloAlwaysAddPadding(t *testing.T) {
	...
	uconnWithoutPadding := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, HelloCustom, false)
	uconnWithPadding := UClient(&net.TCPConn{}, &Config{ServerName: serverName}, HelloCustom, false)
...
func TestUTLSHandshakeClientFingerprintedSpecFromChrome_58(t *testing.T) {
	...
	uconn := UClient(&net.TCPConn{}, originalConfig, helloID, false)
	if err := uconn.BuildHandshakeState(); err != nil {
		t.Errorf("got error: %v; expected to succeed", err)
	}
